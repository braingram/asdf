...
from .exceptions import AsdfCustomTypeDeprecationWarning, AsdfWarning
        ...
        try:
            ...
            with warnings.catch_warnings():
                ...
                warnings.filterwarnings(
                    "ignore",
                    category=AsdfCustomTypeDeprecationWarning,
                    message=r".* from astropy.io.misc.asdf.* subclasses the deprecated CustomType .*",
                )
                # Unfortunately, astropy.io.misc.asdf.tags.transforms.projections is a bit different and
                # modifies the namespace of asdf.types directly. Fortunately, asdf.types does not natively
                # have any such deprecation warnings, so we can filter them out here.
                warnings.filterwarnings(
                    "ignore",
                    category=AsdfCustomTypeDeprecationWarning,
                    message=r".* from asdf.types.* subclasses the deprecated CustomType .*",
                )
